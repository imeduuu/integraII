...
async function main() {
  ...
  const org = await prisma.organizacion.upsert({
    ...
    update: {},
    create: { id_organizacion: 1, nombre_organizacion: 'Fundación Animal Demo', telefono_organizacion: '111111111', email_organizacion: 'contacto@fundaciondemo.com', direccion: 'Av. Siempre Viva 123' },
  });
  ...
  // Voluntario
  await prisma.voluntario.upsert({ where: { id_voluntario: 1 }, update: {}, create: { id_voluntario: 1, id_usuario: 4, fecha_registro: new Date() } });
  ...
  await prisma.google_accounts.upsert({ where: { id_google_account: 1 }, update: {}, create: { id_google_account: 1, id_usuario: 2, google_account_id: 'fake-google-id', google_email: 'user@demo.com', refresh_token: 'fake-refresh-token', access_token: 'fake-access-token' } });

  // Comentarios
  await prisma.comentario.upsert({ where: { id_comentario: 1 }, update: {}, create: { contenido: 'Firulais es muy amigable y juguetón.', id_usuario: 2, id_animal: 1 } });
  ...
  await prisma.faq.upsert({ where: { id_faq: 3 }, update: {}, create: { pregunta: '¿Cómo puedo ser voluntario?', respuesta: '¡Gracias por tu interés! Puedes registrarte como voluntario en la sección de "Voluntariado". Una vez registrado, una organización se pondrá en contacto contigo para coordinar actividades.' } });
}
