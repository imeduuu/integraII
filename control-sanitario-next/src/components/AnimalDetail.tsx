...
const AnimalDetail: React.FC<AnimalDetailProps> = ({ name, status, location, age, images = [], isLoading = false }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md p-6 w-full max-w-2xl mx-auto flex flex-col items-center">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-300 mb-1"><span className="font-semibold">Estado: {status}</span></p>
      <p className="text-sm text-gray-600 dark:text-gray-300 mb-1"><span className="font-semibold">Ubicación: {location}</span></p>
      {age && <p className="text-sm text-gray-600 dark:text-gray-300 mb-2"><span className="font-semibold">Edad: {age}</span></p>}
    </div>
  );
};
